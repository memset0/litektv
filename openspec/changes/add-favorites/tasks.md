...
- [x] 1.2 Add typed accessors in `db.ts`: `listFavorites()`, `findFavorite(source, videoId, page)`, `addFavorite(row)` (idempotent on PK; first starrer wins), `removeFavorite(source, videoId, page)` (anyone can remove).
...
- [x] 3.2 Implement WS handlers `favorite.add`, `favorite.remove`, `favorite.list` in `ws.ts`. `favorite.add` accepts `{source, videoId, page?, title?, thumb?, duration?}` and uses the strict zod schema so unknown keys cause `{type:"error", error:"unknown field"}`. Missing metadata is filled in via `parseRef`. The server stamps `addedBy` from the connection's presence — clients can't supply it.
...
- [x] 4.1 In `packages/frontend/state.jsx`, add a `favorites` store that subscribes to `{type:"favorites"}` messages and exposes `useFavorites()` plus `addFavorite(song)` / `removeFavorite(song)` / `isFavorited(song)` helpers. The list is global so no client-side scoping.
...
- [x] 6.2 Build the catalog modal (`catalog.jsx`): centered dialog with backdrop, search input pinned at the top, list ordered by `added_at` DESC, Esc-to-close + backdrop-click-to-close. Each row shows title / source badge / favoriter (`addedBy.emoji name`) / "+ to queue" action / unstar action.
...
- [x] 8.1 Vitest: parser canonicalization (URL with spm_id_from), `parseRef` round-trip, favorites idempotent add (first-starrer wins), Bilibili page separation, public list visibility, anyone-can-remove. **11 tests passing.**
