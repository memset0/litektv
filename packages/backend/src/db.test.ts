...
const { initDb, addFavorite, listFavorites, findFavorite } = dbModule;
...
describe("favorites are global (site-wide) and add-only", () => {
  it("adding the same song twice keeps one row and preserves the first starrer", () => {
    ...
    const row = findFavorite("bili", "BV_dedupe_1", 0);
  ...
  it("favorites list is shared site-wide; both starrers appear in the same listFavorites() result", () => {
